...
BEGIN
    ...
    -- If we get here, pgvector is available, so create the embeddings table
    ...
    RAISE NOTICE 'pgvector embeddings migration completed successfully';

EXCEPTION
    WHEN OTHERS THEN
        -- Log that pgvector is not available but don't fail
        RAISE NOTICE 'pgvector extension not available, skipping embeddings migration';
END $$;

-- Record this migration as completed regardless of whether pgvector was available
